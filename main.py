...
# ========== ФУНКЦИИ ДЛЯ ЕЖЕНЕДЕЛЬНЫХ ОТЧЕТОВ ==========
async def generate_weekly_report():
    ...
    try:
        ...
        report_reactions = f"""📊 ЕЖЕНЕДЕЛЬНЫЙ ОТЧЕТ: Топ-20 постов по реакциям
{weekday}, {date_str}
...
"""
        for idx, (channel_id, username, title, message_id, reactions, post_date, post_text) in enumerate(reactions_posts, 1):
            ...
            report_reactions += f"{idx}. {title}\n"
            if preview:
                report_reactions += f"   💬 {preview}\n"
            report_reactions += f"   ❤️ {reactions} реакций | Канал: {channel_link} | Пост: {post_link}\n\n"
        
        views_posts = db.get_top_posts_by_views(20)
        report_views = f"""📊 ЕЖЕНЕДЕЛЬНЫЙ ОТЧЕТ: Топ-20 постов по просмотрам
{weekday}, {date_str}
...
"""
        for idx, (channel_id, username, title, message_id, views, post_date, post_text) in enumerate(views_posts, 1):
            ...
            report_views += f"{idx}. {title}\n"
            if preview:
                report_views += f"   💬 {preview}\n"
            report_views += f"   👁️ {views_formatted} просмотров | Канал: {channel_link} | Пост: {post_link}\n\n"
        
        forwards_posts = db.get_top_posts_by_forwards(20)
        report_forwards = f"""📊 ЕЖЕНЕДЕЛЬНЫЙ ОТЧЕТ: Топ-20 постов по репостам
{weekday}, {date_str}
...
"""
        for idx, (channel_id, username, title, message_id, forwards, post_date, post_text) in enumerate(forwards_posts, 1):
            ...
            report_forwards += f"{idx}. {title}\n"
            if preview:
                report_forwards += f"   💬 {preview}\n"
            report_forwards += f"   🔄 {forwards} репостов | Канал: {channel_link} | Пост: {post_link}\n\n"
        
        growth_channels = db.get_top_channels_by_growth('7d', 20)
        report_growth = f"""📊 ЕЖЕНЕДЕЛЬНЫЙ ОТЧЕТ: Топ-20 каналов по росту (за 7 дней)
{weekday}, {date_str}
...
"""
        for idx, (channel_id, username, title, subscribers, growth_7d, growth_30d) in enumerate(growth_channels, 1):
            clean_username = username[1:] if username.startswith('@') else username
            channel_link = f"https://t.me/{clean_username}"
            report_growth += f"{idx}. {title}\n   📈 {growth_7d:+.1f}% | 👥 {subscribers:,} подписчиков | Канал: {channel_link}\n\n"
...
async def send_weekly_report():
    ...
    try:
        ...
        print(f"✅ Еженедельный отчет отправлен в {REPORT_CHANNEL_ID}")
    ...
    try:
        while True:
            ...
            if now.weekday() == 5 and now.hour == 7 and now.minute == 0:
                print("📅 Суббота 7:00 по Владивостоку - отправляю отчет")
